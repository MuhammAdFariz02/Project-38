...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$sql = mysqli_query($konek, "UPDATE jenis_bayar SET jumlah='$jumlah' WHERE th_pelajaran='$tapel' AND tingkat='$tingkat'");
		...
		$sql = mysqli_query($konek, "SELECT * FROM jenis_bayar WHERE th_pelajaran='$tapel' AND tingkat='$tingkat'");
		list($thn,$tk,$jml) = mysqli_fetch_array($sql);
?>
<h2>Edit Jenis Pembayaran</h2>
<hr>
<form method="post" action="admin.php?hlm=master&sub=jenis&aksi=edit" class="form-horizontal" role="form" autocomplete="off">
