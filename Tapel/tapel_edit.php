...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$sql = mysqli_query($konek, "UPDATE tapel SET tapel='$tapel' WHERE id='$id'");
		...
		$id= $_REQUEST['id'];
		$sql = mysqli_query($konek, "SELECT tapel FROM tapel WHERE id='$id'");
		list($tapel) = mysqli_fetch_array($sql);
?>
<h2>Edit Tahun Pelajaran</h2>
<hr>
<form method="post" action="admin.php?hlm=master&sub=tapel&aksi=edit" class="form-horizontal" role="form" autocomplete="off">
